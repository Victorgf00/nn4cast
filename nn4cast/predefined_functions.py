...
def dibujo_1_mapa_cartopy_pixel(A, lon, lat, levs, cmap1, l1, titulo, ax, custom_labels=False):
    """
    Create a pixel-based colormap plot using Cartopy.
    ...
    - titulo (str): Title for the plot.
    - ax (matplotlib.axes.Axes): Matplotlib axis to draw the plot on.
    - custom_labels (bool, optional): Whether to set custom labels on the colorbar (default=False).

    Returns:
    - None

    This function creates a pixel-based colormap plot using Cartopy, displaying geographical data.
    """
    ...
    # Set the custom labels on the colorbar if requested
    if custom_labels:
        custom_labels = [-1.0, -0.6, -0.2, 0.2, 0.6, 1.0]
        cbar.set_ticks(custom_labels)

def data_split(train_years,validation_years,testing_years,predictor,predictant, replace_nans_with_0_predictor=False,replace_nans_with_0_predictant=False):
    """
...
def prediction_vs_observation1D(X_train, Y_train, X_test, Y_test, model1, std_slp, train_years, testing_years, outputs_path):
    """
    ...
    Parameters:
        ...
        std_slp (float): Standardization factor for predictions.
    ...
    prediction_train, observation_train = (model1.predict(X_train)) * np.array(std_slp), Y_train * std_slp
    ...
    prediction_test, observation_test = (model1.predict(X_test)) * np.array(std_slp), Y_test * std_slp
...
def evaluation(model, X_test, Y_test, longitude, latitude, std_slp):
    ...
    - std_slp (float): Standard deviation for sea-level pressure data (for de-standardization).
    ...
    if np.ndim(predicted1.shape)<=2:
        predicted1= np.reshape(predicted1, (len(np.array(latitude)), len(np.array(longitude))))
...
def correlations(predicted1, correct_value, outputs_path, lat_iceland, lon_iceland, lat_macaronesian, lon_macaronesian, time_series=False, index_1=None, index_1_name=None, index_2=None, index_2_name=None):
    ...
    Parameters:
    - predicted1 (xarray.DataArray): Predicted climate data.
    - correct_value (xarray.DataArray): Observed climate data.
    - outputs_path (str): Path to save the generated plots.
    - lat_iceland (list): Latitude range for the Iceland region.
    - lon_iceland (list): Longitude range for the Iceland region.
    - lat_macaronesian (list): Latitude range for the Macaronesian region.
    - lon_macaronesian (list): Longitude range for the Macaronesian region.
    - time_series (bool): Whether to include time series plots.
    - index_1 (array-like): Time series data for an additional index (optional).
    - index_1_name (str): Name of the first additional index (optional).
    - index_2 (array-like): Time series data for a second additional index (optional).
    - index_2_name (str): Name of the second additional index (optional).
    ...
    thresh = 0.05  # Set the significance threshold
    ...
    temporal_correlation_iceland = xr.corr(predictions.sel(latitude=slice(lat_iceland[0], lat_iceland[1]),
                                                        longitude=slice(lon_iceland[0], lon_iceland[1])),
                                          observations.sel(latitude=slice(lat_iceland[0], lat_iceland[1]),
                                                          longitude=slice(lon_iceland[0], lon_iceland[1])),
                                          dim=('longitude', 'latitude'))
    temporal_correlation_macaronesian = xr.corr(predictions.sel(latitude=slice(lat_macaronesian[0], lat_macaronesian[1]),
                                                            longitude=slice(lon_macaronesian[0], lon_macaronesian[1])),
                                                observations.sel(latitude=slice(lat_macaronesian[0], lat_macaronesian[1]),
                                                                longitude=slice(lon_macaronesian[0], lon_macaronesian[1])),
                                                dim=('longitude', 'latitude'))
    ...
    temporal_rmse_iceland = np.sqrt((((predictions.sel(latitude=slice(lat_iceland[0], lat_iceland[1]),
                                                           longitude=slice(lon_iceland[0], lon_iceland[1]))) -
                                     observations.sel(latitude=slice(lat_iceland[0], lat_iceland[1]),
                                                      longitude=slice(lon_iceland[0], lon_iceland[1]))) ** 2).mean(
        dim=('longitude', 'latitude')))
    temporal_rmse_macaronesian = np.sqrt((((predictions.sel(latitude=slice(lat_macaronesian[0], lat_macaronesian[1]),
                                                               longitude=slice(lon_macaronesian[0], lon_macaronesian[1]))) -
                                           observations.sel(latitude=slice(lat_macaronesian[0], lat_macaronesian[1]),
                                                            longitude=slice(lon_macaronesian[0], lon_macaronesian[1]))) ** 2).mean(
        dim=('longitude', 'latitude')))

    # Determine significant pixels
    sig_pixels = np.abs(p_value) <= thresh
    ...
    rango = 1
    dibujo_1_mapa_cartopy_pixel(data, data.longitude, data.latitude, np.linspace(-rango, +rango, 15), 'RdBu_r',
                                'Correlation', 'Temporal Correlation', ax)
    ...
    # Add rectangles for Iceland and Macaronesian regions
    rect = mpatches.Rectangle((lon_iceland[0], lat_iceland[0]), lon_iceland[1] - lon_iceland[0],
                              lat_iceland[1] - lat_iceland[0], linewidth=2, edgecolor='green', facecolor='none',
                              fill=True, transform=ccrs.PlateCarree(), label='Iceland region')
    ax.add_patch(rect)
    rect = mpatches.Rectangle((lon_macaronesian[0], lat_macaronesian[0]), lon_macaronesian[1] - lon_macaronesian[0],
                              lat_macaronesian[1] - lat_macaronesian[0], linewidth=2, edgecolor='orange',
                              facecolor='none', fill=True, transform=ccrs.PlateCarree(), label='Macaronesian region')
    ax.add_patch(rect)
    ...
    data = {'time': temporal_correlation.time, 'Predictions correlation': temporal_correlation,
            'Iceland correlation': temporal_correlation_iceland,
            'Macaronesian correlation': temporal_correlation_macaronesian}
    ...
    if time_series:
        # Create twin axes for time series
        ax2 = ax1.twinx()
        ax3 = ax1.twinx()
    ...
    data = spatial_rmse
    dibujo_1_mapa_cartopy(data, data.longitude, data.latitude, np.linspace(0, np.max(np.array(data)), 20), 'OrRd',
                          'RMSE', 'Temporal RMSE', ax4)
    rect = mpatches.Rectangle((lon_iceland[0], lat_iceland[0]), lon_iceland[1] - lon_iceland[0],
                              lat_iceland[1] - lat_iceland[0], linewidth=2, edgecolor='green', facecolor='none',
                              fill=True, transform=ccrs.PlateCarree(), label='Iceland region')
    ax4.add_patch(rect)
    ...
    data = {'time': temporal_rmse.time, 'Predictions RMSE': temporal_rmse, 'Iceland RMSE': temporal_rmse_iceland,
            'Macaronesian RMSE': temporal_rmse_macaronesian}
    df = pd.DataFrame(data)
    df.set_index('time', inplace=True)
...
def nino_index(lon_index, lat_index, data, time_period, detrend=False):
    """
    Calculate the Niño Index (SST anomaly) for a specified region and time period.

    Parameters:
    - lon_index (list): Longitude range for the region of interest.
    - lat_index (list): Latitude range for the region of interest.
    - data (xarray.DataArray): Sea Surface Temperature (SST) data.
    - time_period (list): Time period [start, end] for which to calculate the Niño Index.
    - detrend (bool): Whether to perform linear detrending on the data before calculation.

    Returns:
    - index (xarray.DataArray): Niño Index (SST anomaly) for the specified region and time period.
    """
    # Select data within the specified region and time period
    data_region = data.sel(latitude=slice(lat_index[0], lat_index[1]),
                           longitude=slice(lon_index[0], lon_index[1]),
                           time=slice(str(time_period[0]), str(time_period[1])))

    if detrend:
        # Perform linear detrending on the data
        adjust, time_step = (data_region.polyfit(dim='time', deg=1)).sel(degree=1).polyfit_coefficients, np.arange(len(data_region))
        data_region = data_region - np.array(adjust) * time_step

    # Calculate the spatial mean of SST for each time and compute the anomaly
    spatial_mean = data_region.mean(dim=('latitude', 'longitude'))
    anomaly = ((spatial_mean.groupby('time.month') - spatial_mean.groupby('time.month').mean('time')) / spatial_mean.groupby('time.month').std('time')).groupby('time.month')

    # Calculate the Niño Index (SST anomaly) for the SON (September, October, November) season
    index = (anomaly['SON']).groupby('time.year').mean()

    return index

def nao_index(azores_location, iceland_location, data, time_period, detrend=False):
    """
    Calculate the North Atlantic Oscillation (NAO) Index based on pressure data.

    Parameters:
    - azores_location (tuple): Latitude and longitude coordinates of the Azores location.
    - iceland_location (tuple): Latitude and longitude coordinates of the Iceland location.
    - data (xarray.DataArray): Pressure data (in Pa).
    - time_period (list): Time period [start, end] for which to calculate the NAO Index.
    - detrend (bool): Whether to perform linear detrending on the NAO Index.

    Returns:
    - NAO (xarray.DataArray): NAO Index.
    """
    escala = 1 / 100  # to convert Pa to hPa
    data = data.sel(time=slice(str(time_period[0]), str(time_period[1])))
    
    # Select data for the Azores and Iceland locations
    Azores_point = data.sel(latitude=azores_location[0], longitude=azores_location[1], method='nearest') * escala
    Iceland_point = data.sel(latitude=iceland_location[0], longitude=iceland_location[1], method='nearest') * escala
    
    # Calculate the pressure difference
    pressure_difference = Azores_point - Iceland_point
    
    if detrend:
        # Perform linear detrending on the NAO Index
        time_step, adjust = np.arange(len(pressure_difference)), (np.polyfit(time_step, pressure_difference, deg=1))[0]
        pressure_difference = pressure_difference - adjust * time_step

    # Calculate the NAO Index as standardized anomalies
    NAO = ((pressure_difference.groupby('time.month') - pressure_difference.groupby('time.month').mean('time')) / pressure_difference.groupby('time.month').std('time')).groupby('time.month')
